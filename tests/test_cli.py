...
from x2fa.init_app.database import database as db_init


@pytest.fixture(scope="module", autouse=True)
def setup_test_db():
    """Create a separate test database for CLI tests."""
    os.environ["X2FA_DATABASE_URL"] = "sqlite:///test_cli.db"
    
    app = create_app()
    with app.app_context():
        from x2fa.init_app.database import _engine
    ...
    db_file = os.path.join(os.getcwd(), "test_cli.db")
...
def test_add_client_new():
    ...
    result = runner.invoke(cli, ["add-client", "test-client", "https://example.com/callback"])
...
def test_add_client_existing_updates():
    ...
    # First add
    result1 = runner.invoke(cli, ["add-client", "demo-rp-updated", "https://old.callback.url/cb"])
    assert result1.exit_code == 0
    
    with create_app().app_context():
        with SessionFactory() as db_session:
            client_obj = db_session.query(OIDCClient).filter_by(client_id="demo-rp-updated").first()
            old_secret = client_obj.client_secret
            old_uri = client_obj.redirect_uris
    
    # Second add (update)
    result2 = runner.invoke(cli, ["add-client", "demo-rp-updated", "https://new.callback.url/cb"])
...
def test_revoke_client_exists():
    ...
    with create_app().app_context():
        with SessionFactory() as db_session:
            client_obj = db_session.query(OIDCClient).filter_by(client_id="to-revoke").first()
