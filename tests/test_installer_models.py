...
class TestInstallConfigDefaults:
    ...
    def test_x2fa_home_defaults_to_home(self):
        config = InstallConfig()
        assert config.x2fa_home == Path.home()

    def test_ca_key_path_derived_from_x2fa_home(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path)
        assert config.ca_key_path == str(tmp_path / ".local" / "share" / "x2fa" / "ca_key.pem")

    def test_ca_cert_path_derived_from_x2fa_home(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path)
        assert config.ca_cert_path == str(tmp_path / ".local" / "share" / "x2fa" / "ca_cert.pem")

    def test_ca_key_path_can_be_overridden(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path, ca_key_path="/custom/ca.key")
        assert config.ca_key_path == "/custom/ca.key"

    def test_ca_cert_path_can_be_overridden(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path, ca_cert_path="/custom/ca.crt")
...
class TestDirectoryHelpers:
    def test_data_dir_under_x2fa_home(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path)
        assert config._data_dir() == tmp_path / ".local" / "share" / "x2fa"

    def test_config_dir_under_x2fa_home(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path)
...
class TestEffectiveDbUri:
    ...
    def test_custom_uri_returned_verbatim(self, tmp_path):
        config = InstallConfig(x2fa_home=tmp_path, db_uri="postgresql://u:p@localhost/x2fa")
...
class TestEffectiveCaCert:
    def test_generate_returns_ca_cert_path_when_file_exists(self, tmp_path):
        ...
        config = InstallConfig(x2fa_home=tmp_path, ca_action="generate",
    ...
    def test_import_returns_ca_import_path_when_file_exists(self, tmp_path):
        ...
        cert.write_text("placeholder")
        config = InstallConfig(x2fa_home=tmp_path, ca_action="import",
