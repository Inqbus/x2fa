...
or X2FA backend.  It verifies that the ExecuteScreen correctly calls:

1. generate_ca() to create a self-signed CA certificate
2. flask add-ca to register the CA in the database  
3. flask issue-client-cert to issue a client certificate
4. flask add-client to register the OIDC client

All operations use the real X2FA CLI and the real cryptography library,
not any mock objects.
"""
...
async def test_e2e_real_ca_generate(tmp_path):
    ...
    ExecuteScreen generates real CA and issues real client cert."""
